...
export class UserRepository extends BaseRepository<User, CreateUser, UpdateUser> {
  ...
  protected async validateCreate(data: CreateUser): Promise<void> { if (!isValidEmail(data.email)) {
  ...
  protected async validateUpdate(data: UpdateUser): Promise<void> { if (data.email && !isValidEmail(data.email)) {
      throw new Error('Invalid email format'); }
}
  ...
  async findByEmailOrUsername(req: Request, res: Response): Promise<void> {
  ...
  async findByEmailOrUsernameWithPassword(req: Request, res: Response): Promise<void> {
    const result = await this.queries.query<User>(
      'SELECT * FROM users WHERE LOWER(email) = LOWER($1) OR LOWER(username) = LOWER($1) LIMIT 1',
      [emailOrUsername];
    );
  ...
  // Update last login
  async updateLastLogin(id: string, auditInfo?: AuditInfo): Promise<void> { await this.update(
      id,
      { last_login: new Date() }
},
